...
async fn dev_node_serves_explorer_page() {
    ...
    assert_eq!(
        ...
        format!(
            "0x{:x}",
            kanari_evm_move_execution::KANARI_EVM_MAX_SUPPLY_ETH * ONE_ETH_WEI
        ),
...
async fn dev_node_answers_batch_requests_like_wallets_send() {
    ...
    let batch = json!([
        ...
        {"jsonrpc": "2.0", "id": 3, "method": "eth_getBalance",
         "params": [kanari_evm_move_execution::DEV_FUNDED_ACCOUNT.to_string(), "latest"]},
    ...
    assert_eq!(
        ...
        kanari_evm_move_execution::DEV_FUNDED_BALANCE
    );
...
async fn block_priority_fees_accrue_to_beneficiary() {
    ...
    assert_eq!(
        node.balance_of(kanari_evm_move_execution::BLOCK_BENEFICIARY)
            .expect("balance"),
        U256::ZERO,
